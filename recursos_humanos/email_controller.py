...
class EmailController:
    ...
    def _search_emails(self, value):
        search_query = self.INBOX_SEARCH + value
        res_threat = ezgmail.search(search_query)
        # print(ezgmail.summary(res_threat, printInfo=False))
        return res_threat

    def download_all_attachments_by_subject(self, value):
        emails = self._search_emails(value)
        for email_index, email_threat in enumerate(emails):
            for reply_index, email_message in enumerate(email_threat.messages):
                ...
                email_message.downloadAllAttachments(downloadFolder=f'cv/{value}_selection_process')

    def download_all_attachments_by_subject_and_format(self, value, file_format='.pdf'):
        emails = self._search_emails(value)
        for email_index, email_threat in enumerate(emails):
            for reply_index, email_message in enumerate(email_threat.messages):
                print(f'Threat: {email_index} - reply: {reply_index}')
                ...
                for attachment in self.filter_by_extension(files=attachments, extension=file_format):
                    email_message.downloadAttachment(filename=attachment,
                                                     downloadFolder=f'cv/{value}_selection_process_{directory_name}')
